...
public class EventsSignaturesCompilationTest {
    ...
    @Listener
    void onKarafBootingEvent(KarafBootingEvent event) {

    }

    @Test
    public void compile() {
        final Subscription sub1 = sub.subscribe(this);

        final Subscription sub2 = sub.subscribe(KarafBootingSuccessfulEvent.class, e -> {
        });

        // make sure we can subscribe a listener of a super type to a sub type event class
        final Subscription sub3 = sub.subscribe(KarafBootingFailedEvent.class, new EventListener<KarafBootingEvent>() {
