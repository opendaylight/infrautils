...
final class ThrowableAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(final Gson gson, final TypeToken<T> type) {
        ...
        final TypeAdapter<T> adapter = (TypeAdapter<T>) new TypeAdapter<Throwable>() {
            @Override
            public Throwable read(final JsonReader in) throws IOException {
                ...
                if (exceptionType != null && !exceptionType.isEmpty()) {
                    try {
                        final Class<?> exceptionClass = Class.forName(exceptionType);
                        final Constructor<?> constructor = exceptionClass.getConstructor(String.class, Throwable.class);
                        final Throwable throwable = (Throwable) constructor.newInstance(message, cause);
