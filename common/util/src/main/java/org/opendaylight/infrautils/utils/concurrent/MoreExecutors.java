...
public class MoreExecutors {
     ...
     *
     * @param namePefix Prefix for this executor thread names
     * @param logger Logger used to log uncaught exceptions
     * @return the newly created single-threaded Executor
     */
    public static ExecutorService newSingleThreadExecutor(final String namePefix, final Logger logger) {
        return Executors.newSingleThreadExecutor(
                ThreadFactoryProvider.builder()
                        .namePrefix(namePefix)
                        .logger(logger)
                        .build()
                        .get());
    }
