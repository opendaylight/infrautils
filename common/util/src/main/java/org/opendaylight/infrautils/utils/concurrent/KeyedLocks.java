...
public class KeyedLocks<T> {
    ...
    private static final class WeakRef<T> extends FinalizableWeakReference<ReentrantLock> {
        private final Map<T, WeakRef<T>> locks;
        private final T key;

        WeakRef(FinalizableReferenceQueue queue, Map<T, WeakRef<T>> locks, T key) {
     ...
     */
    public LockResult tryLock(T lockKey, long timeout, TimeUnit unit) {
     ...
     */
    public LockResult tryLock(T lockKey) {
     ...
     * @param lockKey the key to lock
     */
    public Acquired lock(T lockKey) {
    ...
    private ReentrantLock getLock(T lockKey) {
        while (true) {
            WeakRef<T> ref = locks.computeIfAbsent(lockKey, this::createRef);
    ...
    private WeakRef<T> createRef(T key) {
        return new WeakRef<>(queue, locks, key);
    }
