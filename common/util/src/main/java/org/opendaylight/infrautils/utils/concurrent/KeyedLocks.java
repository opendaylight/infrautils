...
public class KeyedLocks<T> {
    ...
    private static final class WeakRef<T> extends FinalizableWeakReference<ReentrantLock> {
        ...
        WeakRef(final FinalizableReferenceQueue queue, final Map<T, WeakRef<T>> locks, final T key) {
    ...
    private ReentrantLock getLock(final T lockKey) {
        while (true) {
            final WeakRef<T> ref = locks.computeIfAbsent(lockKey, this::createRef);
    ...
    private WeakRef<T> createRef(final T key) {
