...
public class KeyedLocks<T> {
    ...
    private final Map<T, WeakReference<ReentrantLock>> locks = new ConcurrentHashMap<>();
    ...
    private ReentrantLock getLock(final T lockKey) {
        while (true) {
            final WeakReference<ReentrantLock> ref = locks.computeIfAbsent(lockKey, key ->
            new FinalizableWeakReference<ReentrantLock>(new ReentrantLock(), queue) {
                @Override
