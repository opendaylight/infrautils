...
public class KeyedLocks<T> {
    ...
    // Potentially heavy-weight, could be a ConcurrentHashMap with WeakReferences, but then we need to also deal with
    // reference queuing, i.e. calling locks.remove(key, ref) when a reference is cleared.
    @SuppressWarnings("null")
