...
class JobEntry<T> {
    ...
    private volatile List<ListenableFuture<T>> futures;
    private volatile SettableFuture<List<ListenableFuture<T>>> result;

    public ListenableFuture<List<ListenableFuture<T>>> getResult() {
        return result;
    }

    public void setResult(List<ListenableFuture<T>> futureresults) {
        this.result.set(futureresults);
    }
