...
public class JobCoordinatorImpl implements JobCoordinator, JobCoordinatorMonitor {
    ...
    @Override
    public <T> List<Future<T>> enqueueJob(String key, Callable<List<ListenableFuture<T>>> mainWorker) {
    ...
    @Override
    public <T> List<Future<T>> enqueueJob(String key, Callable<List<ListenableFuture<T>>> mainWorker, int maxRetries) {
