...
public interface JobCoordinator { // do *NOT* extends JobCoordinatorMonitor
     ...
     */
    <T> ListenableFuture<List<ListenableFuture<T>>> enqueueJob(String key, Callable<List<ListenableFuture<T>>> mainWorker);

    /**
     * Enqueues a job with a rollback task and DEFAULT_MAX_RETRIES (3) retries..
     * See class level documentation above for details re. the retry strategy.
     *
     * @param rollbackWorker The rollback task which runs in case the job's main task
     *                       fails.
     ...
     * See class level documentation above for details re. the retry strategy.
     *
     * @param maxRetries The maximum number of retries for the job's main task until it
     *                   succeeds.
     * @see JobCoordinator#enqueueJob(String, Callable)
     */
    <T> ListenableFuture<List<ListenableFuture<T>>> enqueueJob(String key, Callable<List<ListenableFuture<T>>> mainWorker,
