...
public class GuavaCacheProvider implements NonDistributedNonTransactionalCacheProvider {
    ...
    public <K, V> Cache<K, V> newCache(CacheConfig<K, V> cacheConfig, CachePolicy cachePolicy) {
        return new DelegatingNullSafeCache<>(
            new CacheGuavaAdapter<>(cacheConfig,
                newCacheBuilder(cacheConfig, cachePolicy).build(new CacheLoader<K, V>() {
                    @Override
    ...
    public <K, V, E extends Exception> CheckedCache<K, V, E> newCheckedCache(
            CheckedCacheConfig<K, V, E> cacheConfig, CachePolicy cachePolicy) {
    ...
    private <K, V> CacheBuilder<K, V> newCacheBuilder(BaseCacheConfig cacheConfig, CachePolicy cachePolicy) {
        ...
        builder.maximumSize(cachePolicy.maxEntries());
        return builder;
    }
