...
public class SampleIntegrationTest {
    ...
    // TODO integrate this with bundle[4]-test to ensure all bundles have finished wiring before test start, if need?

    // TODO Jacoco support, for (only?) build on Sonar, see Netvirt IT
    // TODO EclEmma support for coverage view in Eclipse

    // TODO offer to customize logging with a NICE and SIMPLE API.. Class<?> or String

    private static final Logger LOG = LoggerFactory.getLogger(SampleIntegrationTest.class);

    // NB: The sys.propd. named "maven.repo.local" is the Maven standard one
    ...
    public Option[] config() {
        ...
        try {
            ...
            fail("Found Karaf's main class on the test's classpath; DO NOT have a <dependency> to Karaf in the POM!");
        ...
        // We DO NOT want to use versionAsInProject(), because we DO NOT want to have Karaf as a dependency!
        // TODO come up with a way to read this version from a file on the classpath for easier release management?
        MavenUrlReference nixKarafMvnURL = maven().groupId("org.opendaylight.odlparent")
                .artifactId(karafArtifactId).version("1.8.0-SNAPSHOT").type("tar.gz");
        MavenUrlReference winKarafMvnURL = maven().groupId("org.opendaylight.odlparent")
                .artifactId(karafArtifactId).version("1.8.0-SNAPSHOT").type("zip");
        LOG.info("Karaf used: {}", nixKarafMvnURL.toString());
        ...
        return new Option[] {
            ...
            new KarafDistributionKitConfigurationOption(nixKarafMvnURL, NIX)
                .karafVersion(karafVersion)
                .useDeployFolder(false).unpackDirectory(targetPaxExam),
            new KarafDistributionKitConfigurationOption(winKarafMvnURL, WINDOWS)
                .karafVersion(karafVersion)
                .useDeployFolder(false).unpackDirectory(targetPaxExam),
...
*/
    ...
    @Inject /*@Filter(timeout = 60000)*/ SampleService sampleService;

    @Inject BundleContext probeBundleContext;
    ...
    @Test
    public void testEmptyJustToMakeSureKarafStartedOK() throws Exception {
        LOG.info("info log is not visible, as not enabled");
        LOG.warn("warn log is visible");

//        assertThat(probeBundleContext).named("probeBundleContext").isNotNull();
//        assertThat(probeBundleContext.getBundle().getSymbolicName())
//            .named("probeBundleContext.symbolicName").startsWith("PAXEXAM-PROBE-");
//        assertThat(probeBundleContext.getBundle().getState()).isEqualTo(Bundle.ACTIVE);

        for (ServiceReference<?> serviceReference : probeBundleContext.getAllServiceReferences(null, null)) {
            LOG.error("{}", serviceReference.toString());
        }
...
//    @Test
//    public void testSampleService() throws InterruptedException {
//    }
}
